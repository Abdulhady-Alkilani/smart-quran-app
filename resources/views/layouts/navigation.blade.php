        ...
        <div class="flex justify-between h-16">
            <div class="flex items-center gap-4 xl:gap-8">
                ...
                <div class="hidden lg:flex items-center gap-1">
                    <a href="{{ route('dashboard') }}"
                       class="flex items-center gap-2 px-2 xl:px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200
                    ...
                    @if(Auth::user() && Auth::user()->roles()->where('name', 'admin')->exists())
                        ...
                        <a href="{{ url('/admin') }}"
                           class="flex items-center gap-2 px-2 xl:px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 text-[#C9A84C] hover:bg-[#C9A84C]/10 border border-[#C9A84C]/30 hover:border-[#C9A84C]/50">
            ...
            <div class="hidden lg:flex items-center gap-3 flex-shrink-0">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-[#f8fafc]/70 text-sm font-medium whitespace-nowrap">{{ __('messages.nav.greeting') }} {{ Auth::user()->name }}</span>
                    <form method="POST" action="{{ route('logout') }}" class="flex-shrink-0">
                        @csrf
                        <button type="submit" class="flex items-center gap-1.5 text-sm text-red-400 hover:text-red-300 transition px-3 py-2 rounded-lg hover:bg-white/5 whitespace-nowrap">
