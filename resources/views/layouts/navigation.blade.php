        ...
        <div class="flex justify-between h-16">
            <div class="flex items-center gap-8">
                <a href="{{ route('home') }}" class="flex items-center gap-2">
                    ...
                    <span class="text-2xl font-bold text-[#C9A84C] whitespace-nowrap" style="font-family: 'Amiri', serif;">{{ __('messages.app_name') }}</span>
                </a>
                ...
                <div class="hidden sm:flex items-center gap-1">
                    ...
                    @if(Auth::user() && Auth::user()->roles()->where('name', 'admin')->exists())
                        <span class="w-px h-6 bg-[#1B5E20]/40 mx-2"></span>
                        <a href="{{ url('/admin') }}"
                           class="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 text-[#C9A84C] hover:bg-[#C9A84C]/10 border border-[#C9A84C]/30 hover:border-[#C9A84C]/50">
            ...
            <div class="hidden sm:flex items-center gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-[#f8fafc]/70 text-sm">{{ __('messages.nav.greeting') }} {{ Auth::user()->name }}</span>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="flex items-center gap-1.5 text-sm text-red-400 hover:text-red-300 transition px-3 py-2 rounded-lg hover:bg-white/5">
                ...
                <a href="{{ route('locale.switch', $locale === 'ar' ? 'en' : 'ar') }}"
                   class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium text-[#C9A84C] hover:bg-[#C9A84C]/10 border border-[#C9A84C]/30 hover:border-[#C9A84C]/50 transition-all duration-200">
                    @if($locale === 'ar')
            ...
            <div class="flex items-center sm:hidden gap-2">
                ...
                <a href="{{ route('locale.switch', $locale === 'ar' ? 'en' : 'ar') }}"
                    class="text-sm font-medium text-[#C9A84C] px-2 py-1 rounded border border-[#C9A84C]/30">
                    {{ $locale === 'ar' ? 'EN' : 'عربي' }}
                </a>
