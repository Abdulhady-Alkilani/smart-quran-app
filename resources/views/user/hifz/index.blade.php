<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold gradient-text" style="font-family: 'Amiri', serif;">{{ __('messages.hifz.title') }}</h2>
    </x-slot>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="glass-card p-6 mb-6 text-center relative overflow-hidden">
            ...
            <p class="text-[#f8fafc]/60 text-sm mb-2">{{ __('messages.hifz.blind_mode_desc') }}</p>
        </div>
        ...
        <a href="{{ $firstDueRoute }}" class="glass-card p-5 mb-6 flex items-center justify-between group hover:border-purple-500/40 transition-all duration-300 block">
            <div class="flex items-center gap-4">
                <div class="w-14 h-14 rounded-xl bg-gradient-to-br from-purple-600 to-purple-800 flex items-center justify-center group-hover:scale-110 transition-transform">
                ...
                <div>
                    <div class="font-bold text-purple-400 text-lg">{{ __('messages.hifz.start_quick') }}</div>
                    <div class="text-[#f8fafc]/40 text-sm">{{ __('messages.hifz.start_quick_desc') }}</div>
                </div>
            </div>
            <svg class="w-6 h-6 text-purple-400 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        </a>
        @endif
        ...
        <div class="mb-6">
            <form method="GET" action="{{ route('hifz.index') }}" class="flex items-center gap-3">
                ...
                <select name="surah" onchange="this.form.submit()" class="bg-white/5 border border-white/10 text-[#f8fafc] rounded-lg px-4 py-2 text-sm focus:border-purple-400 focus:ring-purple-400 transition">
        ...
        <div class="glass-card p-8 mb-8 text-center relative overflow-hidden">
            <div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-purple-500 via-[#C9A84C] to-purple-500"></div>
            <div class="text-6xl font-bold text-purple-400 mb-2">{{ $totalDue }}</div>
            <div class="text-[#f8fafc]/60 text-lg">{{ __('messages.hifz.due_ayahs') }}</div>
            @if($totalDue > 0)
            ...
            <p class="text-[#f8fafc]/40 text-sm mt-2">{{ __('messages.hifz.start_hifz') }}</p>
            @endif
        </div>
            ...
            <div class="glass-card p-6 group" style="animation: fadeInUp 0.4s ease-out {{ $index * 0.05 }}s both;">
                <div class="flex justify-between items-start mb-3">
                    <div class="flex items-center gap-3">
                        <span class="w-8 h-8 rounded-full bg-purple-500/20 flex items-center justify-center text-purple-400 text-sm font-bold">{{ $index + 1 }}</span>
                        <div>
                ...
                <div class="text-xl leading-loose mb-4 text-right text-[#f8fafc]/70" style="font-family: 'Noto Naskh Arabic', serif;">{{ $item->ayah->text_uthmani }}</div>
                <div class="flex justify-between items-center">
                    ...
                    <a href="{{ route('hifz.recite', $item->ayah) }}" class="flex items-center gap-2 bg-gradient-to-r from-purple-600 to-purple-800 hover:from-purple-700 hover:to-purple-900 text-white px-6 py-2 rounded-lg transition-all duration-200 transform hover:scale-105 shadow-lg shadow-purple-600/20 opacity-80 group-hover:opacity-100">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z"/></svg>
                        {{ __('messages.hifz.recite_blind') }}
                    </a>
                </div>
            </div>
            @endforeach
